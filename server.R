...
shinyServer(
  ...
  function(input,output)
  {
    data <- reactive({
      ...
      if(is.null(file1))
      {return()}
      read.table(file=file1$datapath, sep=input$sep, header=input$header, stringsAsFactors=input$stringsasfactors,fill = TRUE) 
    })
    ...
    data2 <- reactive({
      ...
      if(is.null(file2))
      {return()}
      read.table(file=file2$datapath, sep=input$sep, header=input$header, stringsAsFactors=input$stringsasfactors, fill = TRUE)
    })
    ...
    output$tb <- renderUI({
      if(is.null(data()))
        h5(tags$img(src="brain.jpg"), height=150, width=300)
      else{
        if(input$f14 == TRUE)
        {
          tabsetPanel(
